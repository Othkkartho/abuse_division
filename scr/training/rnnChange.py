...
class RNN:
    def BiLstmLearn(self, embedding_dim, hidden_units, vocab_size, X_train, X_test, y_train, y_test):
        ...
        mc = ModelCheckpoint('../../data/model/rnn_ch_BiLSTM_best_model.h5', monitor='val_acc', mode='max', verbose=1,
                             save_best_only=True)

        model.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['acc'])
        history = model.fit(X_train, y_train, epochs=4, callbacks=[es, mc], batch_size=64, validation_split=0.2)

        loaded_model = load_model('../../data/model/rnn_ch_BiLSTM_best_model.h5')
    ...
    def LstmLearn(self, embedding_dim, hidden_units, vocab_size, X_train, X_test, y_train, y_test):
        ...
        es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=4)
        mc = ModelCheckpoint('../../data/model/rnn_ch_LSTM_best_model.h5', monitor='val_acc', mode='max', verbose=1,
                             save_best_only=True)

        model.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['acc'])
        history = model.fit(X_train, y_train, epochs=20, callbacks=[es, mc], batch_size=64, validation_split=0.2)

        loaded_model = load_model('../../data/model/rnn_ch_LSTM_best_model.h5')
